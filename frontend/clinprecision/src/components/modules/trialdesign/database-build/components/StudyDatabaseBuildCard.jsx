 ...
 */
const StudyDatabaseBuildCard = ({ build, onRefresh, onBuildUpdated }) => {
    ...
    const handleRetry = async () => {
        setShowActionsMenu(false);
        try {
            await studyDatabaseBuildService.retryBuild(build.buildRequestId);
            if (onBuildUpdated) {
                await onBuildUpdated();
            }
        } catch (error) {
    ...
    const handleValidate = async () => {
        ...
        try {
            await studyDatabaseBuildService.validateDatabase(build.buildRequestId);
            if (onBuildUpdated) {
                await onBuildUpdated();
            }
        } catch (error) {
