...
const NumberInput = ({
    ...
    referenceRange,
    context = 'general',
    fieldType = 'number', // 'number', 'integer', 'float'
    validation = {},
    className = '',
    style = {},
    ...props
    ...
    const parseNumericValue = (displayVal) => {
                    ...
                    {showStepControls && !readOnly && !disabled && (
                        <div className="flex flex-col border-l border-gray-300">
                            <button
                                ...
                                disabled={max !== undefined && parseNumericValue(internalValue) >= max}
                            >
                            ...
                            <button
                                ...
                                disabled={min !== undefined && parseNumericValue(internalValue) <= min}
                            >
            ...
            {showReferenceRange && fieldValue !== undefined && fieldValue !== null && (
                <div className="mt-2">
                    ...
                    <div className="relative w-full bg-gray-200 rounded-full h-2">
                        ...
                        <div
                            className={`absolute w-1 h-4 -mt-1 rounded-full ${fieldValue < referenceRange.min ? 'bg-yellow-500' :
                                    fieldValue > referenceRange.max ? 'bg-red-500' :
                                        'bg-green-500'
                                }`}
                            style={{
                                left: `${Math.max(0, Math.min(100, ((fieldValue - (referenceRange.min || 0)) /
...
NumberInput.propTypes = {
    ...
    referenceRange: PropTypes.shape({
        min: PropTypes.number,
        max: PropTypes.number
    }),
    context: PropTypes.oneOf(['general', 'study', 'template', 'patient']),
    fieldType: PropTypes.oneOf(['number', 'integer', 'float']),
