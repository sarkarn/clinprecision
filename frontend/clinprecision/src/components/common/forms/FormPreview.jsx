...
const FormPreview = ({
    formDefinition,
    context = 'general',
    mode = 'interactive', // 'interactive', 'static', 'filled'
    sampleData = {},
    ...
    const getSampleTextValue = (field) => {
        const samples = {
            'first_name': 'John',
            'last_name': 'Doe',
            'name': 'John Doe',
    ...
    const getSampleNumberValue = (field) => {
        const fieldName = field.name?.toLowerCase() || field.id?.toLowerCase() || '';
        ...
        if (fieldName.includes('temperature')) return (Math.random() * 5 + 36).toFixed(1);
    ...
    const getSampleDateValue = (field) => {
        const today = new Date();
        const fieldName = field.name?.toLowerCase() || field.id?.toLowerCase() || '';

        if (fieldName.includes('birth') || fieldName.includes('dob')) {
            // Birth date - random age between 18 and 80
    ...
    const getSampleTextareaValue = (field) => {
        const samples = {
    ...
    const handlePreviewDataChange = (newData) => {
    ...
    const renderFormStructure = () => {
        ...
        const renderFieldsList = (fields, sectionName = '') => (
...
FormPreview.propTypes = {
    ...
    mode: PropTypes.oneOf(['interactive', 'static', 'filled']),
    sampleData: PropTypes.object,
    showDevicePreview: PropTypes.bool,
