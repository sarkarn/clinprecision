...
interface ValidationResultMap {
    [phaseId: string]: ValidationResult;
}
...
interface PhaseTransitionHelperProps {
    currentPhase: string;
    targetPhase?: string;
 ...
 */
const PhaseTransitionHelper: FC<PhaseTransitionHelperProps> = ({
    currentPhase,
    targetPhase,
    completedPhases,
    designProgress,
    ...
    const phaseConfig: PhaseConfigMap = {
        'basic-info': {
    ...
    const validatePhase = async (phaseId: string): Promise<ValidationResult> => {
        setIsValidating(true);

        // Simulate validation API call
        await new Promise(resolve => setTimeout(resolve, 1500));

        const mockValidation: ValidationResultMap = {
            'basic-info': {
                passed: true,
                score: 95,
                issues: [],
