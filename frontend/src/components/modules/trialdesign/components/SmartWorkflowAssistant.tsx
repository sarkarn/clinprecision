...
interface Tip {
  id: string;
  type: 'best-practice' | 'optimization' | 'safety' | 'compliance' | 'quality' | 'enhancement' | 'validation' | 'process';
  title: string;
  content: string;
  action?: string;
}
...
interface Suggestion {
  type: 'completion' | 'next-phase' | 'required';
...
interface SmartWorkflowAssistantProps {
  currentPhase: string;
...
type TabType = 'suggestions' | 'checklist' | 'tips';

/**
 * Smart Workflow Assistant
 * Provides contextual guidance, suggestions, and workflow optimization tips
 */
const SmartWorkflowAssistant: FC<SmartWorkflowAssistantProps> = ({
  currentPhase,
  designProgress,
  completedPhases,
  study,
  ...
  const phaseGuidance: Record<string, PhaseGuidanceData> = {
    'basic-info': {
      icon: FileText,
      color: 'blue',
  ...
  const getCurrentGuidance = (): PhaseGuidanceData => {
  ...
  const getSmartSuggestions = (): Suggestion[] => {
    ...
    if (!isCurrentPhaseCompleted) {
      ...
      const getNextPhaseId = (): string | null => {
    ...
    if (completedPhases.includes(currentPhase) && nextPhase) {
      ...
      if (nextGuidance) {
        suggestions.push({
          ...
          action: () => onPhaseChange && onPhaseChange(nextPhase),
  ...
  const getTipStyling = (type: Tip['type']): string => {
    const styles: Record<Tip['type'], string> = {
    ...
    <div className={`bg-white rounded-xl shadow-sm border border-gray-200 ${className}`}>
      ...
      <div className="p-6 border-b border-gray-200">
        ...
        <div className="mt-4 flex space-x-1 bg-gray-100 rounded-lg p-1">
          {[
            { id: 'suggestions' as TabType, label: 'Smart Suggestions', icon: Zap },
            { id: 'checklist' as TabType, label: 'Checklist', icon: CheckCircle2 },
            { id: 'tips' as TabType, label: 'Best Practices', icon: Lightbulb }
