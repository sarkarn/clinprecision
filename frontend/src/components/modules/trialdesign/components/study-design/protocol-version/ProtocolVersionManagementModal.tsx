...
import useProtocolVersioning, { UseProtocolVersioningReturn } from '../../../../../../../src/hooks/api/studies/useProtocolVersioning';

type BaseProtocolVersion = UseProtocolVersioningReturn['protocolVersions'][number];
type TimelineProps = React.ComponentProps<typeof ProtocolVersionTimeline>;
type TimelineVersion = NonNullable<TimelineProps['versions']>[number];

type ProtocolStatusExtras =
    | 'PROTOCOL_REVIEW'
    | 'AMENDMENT_REVIEW'
    | 'ACTIVE'
    | 'SUPERSEDED'
...
type ProtocolVersion = Omit<BaseProtocolVersion, 'status' | 'amendmentType'> & {
    status?: BaseProtocolVersion['status'] | ProtocolStatusExtras | string;
    statusInfo?: {
        label?: string;
        color?: string;
        canEdit?: boolean;
...
const TIMELINE_ALLOWED_STATUSES: TimelineVersion['status'][] = [
    'DRAFT',
    'UNDER_REVIEW',
    'PROTOCOL_REVIEW',
    'AMENDMENT_REVIEW',
...
type ModalMode = 'manage' | 'create' | 'edit' | 'view';
type TabKey = 'overview' | 'create' | 'edit' | 'timeline';
type FormMode = 'create' | 'edit';

interface ProtocolVersionManagementModalProps {
    isOpen: boolean;
    onClose: () => void;
    studyId: string;
    studyName?: string;
    ...
    onVersionCreated?: (version: any) => Promise<void> | void;
    onVersionUpdated?: (versionId: string | number) => Promise<void> | void;
}
 ...
 */
const ProtocolVersionManagementModal: React.FC<ProtocolVersionManagementModalProps> = ({
    isOpen,
    onClose,
    studyId,
    studyName = '',
    ...
    const uiProtocolVersions = protocolVersions as ProtocolVersion[];
    const uiCurrentProtocolVersion = currentProtocolVersion as ProtocolVersion | null;

    const normalizeTimelineVersion = (version: ProtocolVersion): TimelineVersion => {
        const tentativeStatus = String(version.status ?? 'DRAFT') as TimelineVersion['status'];
        const status = TIMELINE_ALLOWED_STATUSES.includes(tentativeStatus)
            ? tentativeStatus
            : 'DRAFT';

        return {
            ...version,
            id: version.id ?? (version as any).uuid ?? (version as any).studyUuid ?? version.versionNumber ?? '',
            status
        } as TimelineVersion;
    };

    const timelineVersions = uiProtocolVersions.map(normalizeTimelineVersion);
    ...
    const [versionToDelete, setVersionToDelete] = useState<ProtocolVersion | null>(null);

    const normalizeVersionId = (id: string | number | null | undefined): string | null => {
    ...
    const findVersionById = (versionId: string | number | null | undefined): ProtocolVersion | undefined => {
        const normalizedId = normalizeVersionId(versionId);
        if (normalizedId === null) {
            return undefined;
        }

        return uiProtocolVersions.find((version) => normalizeVersionId(version.id) === normalizedId);
    };

    // Initialize selected version
    useEffect(() => {
        if (initialVersionId && uiProtocolVersions.length > 0) {
            const version = findVersionById(initialVersionId);
            setSelectedVersion(version || uiCurrentProtocolVersion);
        } else {
            setSelectedVersion(uiCurrentProtocolVersion);
        }
    }, [initialVersionId, uiProtocolVersions, uiCurrentProtocolVersion]);
    ...
    const handleCreateVersion = async (formData: any): Promise<void> => {
        try {
    ...
    const handleUpdateVersion = async (formData: any): Promise<void> => {
        if (!selectedVersion) return;

        try {
            await updateProtocolVersion(selectedVersion.id as string | number, formData);
            // Call the callback and wait for parent to refresh data
            if (onVersionUpdated) {
                await onVersionUpdated(selectedVersion.id as string | number);
            }
            // Close modal after parent has refreshed
            onClose();
        } catch (error) {
    ...
    const handleDeleteVersion = (versionId: string | number): void => {
        const version = findVersionById(versionId);
        if (version) {
    ...
    const handleConfirmDelete = async (): Promise<void> => {
        if (versionToDelete) {
            try {
                if (versionToDelete.id !== undefined && versionToDelete.id !== null) {
                    await deleteProtocolVersion(versionToDelete.id);
                }
                setShowConfirmDelete(false);
                setVersionToDelete(null);
                if (normalizeVersionId(selectedVersion?.id) === normalizeVersionId(versionToDelete.id)) {
                    setSelectedVersion(uiCurrentProtocolVersion);
                }
            } catch (error) {
    ...
    const handleTimelineVersionSelect = (version: TimelineVersion): void => {
        const matchedVersion = findVersionById(version.id ?? (version as any).uuid ?? null);
        setSelectedVersion(matchedVersion || (version as ProtocolVersion));
    };
    ...
    const isInitialVersion = uiProtocolVersions.length === 0;
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                                ...
                                                <div className="bg-white p-4 border border-gray-200 rounded-lg">
                                                    ...
                                                    <div className="text-2xl font-bold text-gray-900 mt-2">
                                                        {uiProtocolVersions.filter(v => String(v.status) === 'DRAFT').length}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="bg-white p-4 border border-gray-200 rounded-lg">
                                                    ...
                                                    <div className="text-2xl font-bold text-gray-900 mt-2">
                                                        {uiProtocolVersions.filter(v => String(v.status) === 'PROTOCOL_REVIEW').length}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <ProtocolVersionTimeline
                                                    versions={timelineVersions.slice(0, 3)}
                                                    currentVersionId={selectedVersion?.id as string | number | null | undefined}
                            ...
                            {activeTab === 'edit' && selectedVersion && (
                                <div className="p-6">
                                    <ProtocolVersionForm
                                        ...
                                        amendmentTypes={Object.values(AMENDMENT_TYPES)}
                                        loading={loading}
                                        isInitialVersion={selectedVersion.amendmentType === 'MINOR' && uiProtocolVersions.length <= 1}
                                    />
                                </div>
                            )}
                        </div>
