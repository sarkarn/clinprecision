...
interface Comment {
    ...
    content: string;
    timestamp: string;
    type: 'APPROVAL' | 'INFO' | 'CONCERN' | 'REJECTION';
}
...
interface EnhancedRevision extends Revision {
...
interface ApprovalWorkflowInterfaceProps {
    ...
    onAddComment: (revisionId: string, comment: CommentSubmitData) => void;
    isVisible: boolean;
    onClose: () => void;
}

type WorkflowFilterType = 'all' | 'pending' | 'in-review' | 'approved' | 'rejected';
type SortByType = 'priority' | 'date' | 'type' | 'impact';

/**
 * Approval Workflow Interface Component
 * Enhanced workflow management for protocol revision approvals
 */
const ApprovalWorkflowInterface: FC<ApprovalWorkflowInterfaceProps> = ({
    revisions,
    currentUser,
    onApprove,
    onReject,
    ...
    const [selectedRevision, setSelectedRevision] = useState<EnhancedRevision | null>(null);
    ...
    const getEnhancedRevisions = (): EnhancedRevision[] => {
    ...
    const getPriority = (revision: Revision): 'HIGH' | 'MEDIUM' | 'LOW' => {
    ...
    const getAgeScore = (dateString: string): number => {
    ...
    const getEstimatedCompletion = (revision: Revision): string => {
    ...
    const getBottlenecks = (revision: Revision): Bottleneck[] => {
    ...
    const getFilteredRevisions = (): EnhancedRevision[] => {
        let filtered = getEnhancedRevisions();

        if (workflowFilter !== 'all') {
            filtered = filtered.filter(revision => {
                const statusMap: Record<WorkflowFilterType, string> = {
                    all: '',
                    pending: 'DRAFT',
    ...
    // Get priority color
    const getPriorityColor = (priority: EnhancedRevision['priority']): string => {
        switch (priority) {
            case 'HIGH':
    ...
    const getCommentTypeIcon = (type: Comment['type']): React.ReactElement => {
        switch (type) {
            case 'APPROVAL':
    ...
    const handleCommentSubmit = (e: React.FormEvent<HTMLFormElement>): void => {
        e.preventDefault();
        if (!comment.trim() || !selectedRevision) return;
                        ...
                        <div className="flex items-center space-x-4">
                            <div className="flex items-center space-x-2">
                                ...
                                <select
                                    ...
                                    onChange={(e) => setWorkflowFilter(e.target.value as WorkflowFilterType)}
