...
const protocolDeviationService = ProtocolDeviationService as any;
...
const DeviationList: React.FC<DeviationListProps> = ({
    ...
    const [expandedDeviation, setExpandedDeviation] = useState<number | string | null>(null);
    const [loadingComments, setLoadingComments] = useState<Record<number | string, boolean>>({});
    const [comments, setComments] = useState<Record<number | string, Comment[]>>({});
    ...
    const toggleDeviation = async (deviationId: number | string) => {
    ...
    const loadComments = async (deviationId: number | string) => {
        ...
        try {
            ...
            setComments(prev => ({ ...prev, [deviationId]: deviationComments as any }));
        } catch (error) {
                                    ...
                                    <div className="flex items-start gap-3 flex-1">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${protocolDeviationService.getSeverityBadgeClass(deviation.severity as any)}`}>
                                                    {deviation.severity}
                                                </span>
                                                <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${protocolDeviationService.getStatusBadgeClass(deviation.status as any)}`}>
                                            ...
                                            <div className="flex flex-wrap gap-4 text-xs text-gray-600">
                                                <span>
                                                    <strong>Type:</strong> {protocolDeviationService.getDeviationTypeLabel(deviation.deviationType as any)}
                                                </span>
                                                <span>
