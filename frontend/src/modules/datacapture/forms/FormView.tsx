...
interface FormField {
  id: string;
  label: string;
  type: 'text' | 'textarea' | 'number' | 'date' | 'time' | 'radio' | 'checkbox' | 'select' | 'multiselect';
...
const FormView: React.FC = () => {
  ...
  const renderFieldValue = (field: FormField): JSX.Element => {
    ...
    switch (field.type) {
      ...
      case 'radio':
        const radioOption = field.metadata?.options?.find((opt: FieldOption) => opt.value === value);
      ...
      case 'select':
        const selectOption = field.metadata?.options?.find((opt: FieldOption) => opt.value === value);
      ...
      case 'multiselect':
        ...
        const selectedOptions = field.metadata?.options
          ?.filter((opt: FieldOption) => value.includes(opt.value))
