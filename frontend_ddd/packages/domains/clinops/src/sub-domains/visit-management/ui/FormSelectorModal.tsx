...
interface FormSelectorModalProps {
    ...
    onClose: () => void;
    studyId: number;
    visitInstanceId: number; // Long primary key
    visitType?: string; // SCREENING, ENROLLMENT, etc.
    onFormsAdded?: (formIds: string[]) => void;
}
...
const FormSelectorModal: React.FC<FormSelectorModalProps> = ({
    ...
    const loadAvailableForms = async (): Promise<void> => {
     ...
     */
    const toggleFormSelection = (formId: string): void => {
     ...
     */
    const useStandardForms = (): void => {
        const standardFormNames = visitType ? (STANDARD_FORM_SETS[visitType] || []) : [];

        // Find matching forms by name
        const matchingFormIds = availableForms
            .filter(form => standardFormNames.some(config =>
                form.name?.toLowerCase().includes(config.name?.toLowerCase() || '') ||
                form.type?.toLowerCase().includes(config.name?.toLowerCase() || '')
            ))
            .map(form => form.id!)
    ...
    const handleSubmit = async (): Promise<void> => {
        ...
        try {
            ...
            // Convert string IDs to numbers for the helper function
            const numericFormIds = selectedFormIds.map(id => parseInt(id, 10));

            // Use helper utility to assign forms
            await addMultipleFormsToVisit(visitInstanceId, numericFormIds);
