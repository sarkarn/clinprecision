...
@FeignClient(name = "study-design-ws", path = "/api/form-definitions")
public interface FormDefinitionServiceClient {

    @GetMapping("/{id}")
    @Retry(name="study-design-ws")
    @CircuitBreaker(name="study-design-ws", fallbackMethod="getFormDefinitionByIdFallback")
    public ResponseEntity<FormDefinitionDto> getFormDefinitionById(@PathVariable Long id, @RequestHeader("Authorization") String authorization);

    default ResponseEntity<FormDefinitionDto> getFormDefinitionByIdFallback(@PathVariable Long id, String authorization, Throwable exception) {
