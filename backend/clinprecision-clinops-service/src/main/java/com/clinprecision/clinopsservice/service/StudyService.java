...
import com.clinprecision.clinopsservice.exception.StudyNotFoundException;
import com.clinprecision.clinopsservice.mapper.StudyMapper;
import com.clinprecision.clinopsservice.repository.OrganizationStudyRepository;
...
public class StudyService {
    ...
    public StudyResponseDto updateStudy(Long id, StudyUpdateRequestDto request) {
        ...
        // LOG STUDY STATUS BEFORE UPDATE
        logger.info("BEFORE update - Study {} status: {} (status_id: {}), DTO statusId: {}", 
                    id, 
                    existingStudy.getStudyStatus() != null ? existingStudy.getStudyStatus().getCode() : "NULL",
                    existingStudy.getStudyStatus() != null ? existingStudy.getStudyStatus().getId() : "NULL",
                    request.getStudyStatusId());
        ...
        // LOG STUDY STATUS BEFORE SAVE
        logger.info("BEFORE save - Study {} status: {} (status_id: {})", 
                    id, 
                    existingStudy.getStudyStatus() != null ? existingStudy.getStudyStatus().getCode() : "NULL",
                    existingStudy.getStudyStatus() != null ? existingStudy.getStudyStatus().getId() : "NULL");
        ...
        // LOG STUDY STATUS AFTER SAVE
        logger.info("AFTER save - Study {} status: {} (status_id: {})", 
                    id, 
                    updatedStudy.getStudyStatus() != null ? updatedStudy.getStudyStatus().getCode() : "NULL",
                    updatedStudy.getStudyStatus() != null ? updatedStudy.getStudyStatus().getId() : "NULL");
        
        // ALERT IF STATUS CLEARED
        if (existingStudy.getStudyStatus() != null && updatedStudy.getStudyStatus() == null) {
            logger.error("⚠️ CRITICAL: Study {} status was cleared during save operation!", id);
        }
