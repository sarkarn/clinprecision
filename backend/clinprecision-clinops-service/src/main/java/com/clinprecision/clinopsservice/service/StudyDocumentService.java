    ...
    public DocumentStatistics getDocumentStatistics(Long studyId) {
        ...
        try {
            ...
            if (result instanceof Object[]) {
                stats = (Object[]) result;
                logger.debug("Statistics query returned Object[] with {} elements", stats.length);
            } else if (result instanceof java.util.List) {
                java.util.List<?> list = (java.util.List<?>) result;
                if (!list.isEmpty() && list.get(0) instanceof Object[]) {
                    stats = (Object[]) list.get(0);
                    logger.debug("Statistics query returned List<Object[]>, using first element with {} elements", stats.length);
                }
            }
