...
-- =====================================================================================================================
-- TABLE 1: study_field_metadata
-- 
-- Purpose: Store clinical and regulatory metadata for every form field
-- Partition: By study_id (HASH, 16 partitions) for multi-tenant scalability
-- 
-- Features:
--   - Clinical flags (SDV required, medical review, critical data point, etc.)
--   - Regulatory flags (FDA required, EMA required, 21 CFR Part 11, etc.)
--   - Data validation rules (ranges, required checks, derivation logic)
--   - Audit trail flags (full audit, electronic signature, etc.)
-- =====================================================================================================================
CREATE TABLE IF NOT EXISTS study_field_metadata (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    study_id BIGINT NOT NULL,
    form_id BIGINT NOT NULL,
    field_name VARCHAR(100) NOT NULL,
    ...
    -- Clinical Flags
    sdv_required BOOLEAN DEFAULT FALSE COMMENT 'Source Data Verification required',
    medical_review_required BOOLEAN DEFAULT FALSE COMMENT 'Medical review required by physician',
    data_review_required BOOLEAN DEFAULT FALSE COMMENT 'Data manager review required',
    critical_data_point BOOLEAN DEFAULT FALSE COMMENT 'Critical for primary/secondary endpoints',
    safety_data_point BOOLEAN DEFAULT FALSE COMMENT 'Safety-related data (AEs, labs, vitals)',
    efficacy_data_point BOOLEAN DEFAULT FALSE COMMENT 'Efficacy-related data (endpoints)',
    
    -- Regulatory Flags
    fda_required BOOLEAN DEFAULT FALSE COMMENT 'Required by FDA regulations',
    ema_required BOOLEAN DEFAULT FALSE COMMENT 'Required by EMA regulations',
    cfr21_part11 BOOLEAN DEFAULT FALSE COMMENT '21 CFR Part 11 electronic records compliance',
    gcp_required BOOLEAN DEFAULT FALSE COMMENT 'Good Clinical Practice requirement',
    hipaa_protected BOOLEAN DEFAULT FALSE COMMENT 'HIPAA protected health information',
    
    -- Audit Trail Configuration
    audit_trail_level ENUM('NONE', 'BASIC', 'FULL', 'DETAILED') DEFAULT 'BASIC',
    electronic_signature_required BOOLEAN DEFAULT FALSE,
    reason_for_change_required BOOLEAN DEFAULT FALSE,
    
    -- Validation Rules (stored as JSON for flexibility)
    validation_rules JSON COMMENT 'Range checks, pattern matching, cross-field validation',
    
    -- Data Quality Flags
    is_derived_field BOOLEAN DEFAULT FALSE COMMENT 'Field value is calculated/derived',
    derivation_formula TEXT COMMENT 'Formula or logic for derived fields',
    is_query_enabled BOOLEAN DEFAULT TRUE COMMENT 'Allow data queries on this field',
    is_editable_after_lock BOOLEAN DEFAULT FALSE COMMENT 'Can edit after database lock',
    ...
    -- Indexes
    INDEX idx_study_form (study_id, form_id),
    INDEX idx_field_name (study_id, form_id, field_name),
    INDEX idx_sdv_required (study_id, sdv_required),
    INDEX idx_critical_fields (study_id, critical_data_point),
...
-- PARTITION BY HASH(study_id) PARTITIONS 16
COMMENT='Field-level clinical and regulatory metadata for study forms';

-- =====================================================================================================================
-- TABLE 2: study_cdash_mappings
-- 
-- Purpose: Map form fields to CDISC CDASH/SDTM standards
-- Partition: By study_id (HASH, 16 partitions)
-- 
-- Features:
--   - CDASH domain and variable mapping
--   - SDTM transformation rules
--   - Controlled terminology references
--   - ADaM dataset preparation
-- =====================================================================================================================
CREATE TABLE IF NOT EXISTS study_cdash_mappings (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    study_id BIGINT NOT NULL,
    form_id BIGINT NOT NULL,
    field_name VARCHAR(100) NOT NULL,
    ...
    -- CDASH Mapping
    cdash_domain VARCHAR(10) COMMENT 'CDASH domain (VS, DM, AE, CM, etc.)',
    cdash_variable VARCHAR(50) COMMENT 'CDASH variable name',
    cdash_label VARCHAR(255) COMMENT 'CDASH variable label',
    
    -- SDTM Mapping
    sdtm_domain VARCHAR(10) COMMENT 'SDTM domain',
    sdtm_variable VARCHAR(50) COMMENT 'SDTM variable name',
    sdtm_datatype VARCHAR(20) COMMENT 'SDTM data type (TEXT, NUM, DATE, DATETIME)',
    sdtm_length INT COMMENT 'SDTM field length',
    
    -- Controlled Terminology
    cdisc_terminology_code VARCHAR(50) COMMENT 'CDISC CT code (C25473, etc.)',
	data_origin VARCHAR(20) NULL COMMENT 'Data origin: COLLECTED, DERIVED, PROTOCOL, etc.',
    codelist_name VARCHAR(100) COMMENT 'Controlled terminology codelist',
    codelist_version VARCHAR(20) COMMENT 'CT version (2023-12-15)',
    
    -- Transformation Rules
    transformation_rule TEXT COMMENT 'Logic to transform raw data to SDTM',
    unit_conversion_rule TEXT COMMENT 'Unit conversion logic (e.g., lbs to kg)',
    
    -- Origin Metadata
    origin VARCHAR(50) DEFAULT 'CRF' COMMENT 'COLLECTED, DERIVED, ASSIGNED, PROTOCOL',
    ...
    -- Indexes
    INDEX idx_study_form (study_id, form_id),
    INDEX idx_cdash_domain (study_id, cdash_domain),
    INDEX idx_sdtm_domain (study_id, sdtm_domain),
    INDEX idx_field_mapping (study_id, form_id, field_name),
    ...
    -- Unique Constraint
    UNIQUE KEY uk_study_form_field_cdash (study_id, form_id, field_name)
) ENGINE=InnoDB 
-- PARTITION BY HASH(study_id) PARTITIONS 16
COMMENT='CDISC CDASH/SDTM mappings for regulatory submissions';

-- =====================================================================================================================
-- TABLE 3: study_medical_coding_config
-- 
-- Purpose: Configure medical coding dictionaries and auto-coding settings
-- Partition: By study_id (HASH, 16 partitions)
-- 
-- Features:
--   - Medical dictionary configuration (MedDRA, WHO-DD, SNOMED, ICD-10)
--   - Auto-coding settings and thresholds
--   - Dictionary version management
--   - Coding workflow configuration
-- =====================================================================================================================
CREATE TABLE IF NOT EXISTS study_medical_coding_config (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    study_id BIGINT NOT NULL,
    form_id BIGINT NOT NULL,
    field_name VARCHAR(100) NOT NULL,
    ...
    dictionary_type ENUM('MedDRA', 'WHO-DD', 'SNOMED', 'ICD-10', 'ICD-11', 'LOINC', 'Custom') NOT NULL,
    dictionary_version VARCHAR(20) NOT NULL COMMENT 'e.g., MedDRA 26.0, ICD-10-CM 2024',
    dictionary_language VARCHAR(10) DEFAULT 'en' COMMENT 'ISO language code',
    
    -- Coding Requirements
    coding_required BOOLEAN DEFAULT TRUE,
    auto_coding_enabled BOOLEAN DEFAULT FALSE,
    auto_coding_threshold DECIMAL(5,2) COMMENT 'Confidence threshold for auto-coding (0.00-100.00)',
    manual_review_required BOOLEAN DEFAULT TRUE COMMENT 'Require human review even for auto-coded terms',
    
    -- Hierarchy Configuration (for MedDRA, SNOMED)
    code_to_level ENUM('PT', 'LLT', 'HLT', 'HLGT', 'SOC') COMMENT 'MedDRA: Level to code to',
	capture_primary_soc BOOLEAN DEFAULT FALSE NOT NULL COMMENT 'Whether primary System Organ Class (SOC) should be captured',
	show_all_matches BOOLEAN DEFAULT FALSE NOT NULL COMMENT 'Whether all matching codes should be displayed',
	max_matches_displayed INT DEFAULT 10 COMMENT 'Maximum number of matches to display in auto-suggest', 
    include_hierarchy BOOLEAN DEFAULT TRUE COMMENT 'Include full hierarchy in output',
    
    -- Coding Workflow
    primary_coder_role VARCHAR(50) COMMENT 'Role responsible for primary coding',
    secondary_coder_role VARCHAR(50) COMMENT 'Role for secondary/verification coding',
    adjudication_required BOOLEAN DEFAULT FALSE COMMENT 'Require adjudication for discrepancies',
	adjudicator_role VARCHAR(50) NULL COMMENT 'Role for adjudication (e.g., MEDICAL_MONITOR, SAFETY_PHYSICIAN)',
	workflow_type VARCHAR(30) DEFAULT 'SINGLE_CODER' COMMENT 'Coding workflow type: SINGLE_CODER, DUAL_CODER, AUTO_WITH_REVIEW, CENTRALIZED',
	validation_rules TEXT NULL COMMENT 'Custom validation rules for coded values (JSON format)',
	coding_instructions TEXT NULL COMMENT 'Configuration notes or instructions for coders',
	is_active BOOLEAN DEFAULT TRUE NOT NULL COMMENT 'Whether this configuration is active', 
    
    -- Verbatim Text Configuration
    verbatim_field_label VARCHAR(255) COMMENT 'Label for verbatim text field',
    verbatim_max_length INT DEFAULT 500,
    verbatim_required BOOLEAN DEFAULT TRUE,
    
    -- Synonyms and Alternatives
    allow_synonyms BOOLEAN DEFAULT TRUE,
    synonym_sources JSON COMMENT 'Additional synonym dictionaries to use',
    
    -- Output Configuration
    output_code_field VARCHAR(100) COMMENT 'Field name to store coded value',
    output_term_field VARCHAR(100) COMMENT 'Field name to store preferred term',
    output_hierarchy_field VARCHAR(100) COMMENT 'Field name to store full hierarchy',
    ...
    -- Indexes
    INDEX idx_study_form (study_id, form_id),
    INDEX idx_dictionary (study_id, dictionary_type, dictionary_version),
    INDEX idx_field_name (study_id, form_id, field_name),
    ...
    -- Unique Constraint
    UNIQUE KEY uk_study_form_field_coding (study_id, form_id, field_name)
) ENGINE=InnoDB 
-- PARTITION BY HASH(study_id) PARTITIONS 16
COMMENT='Medical coding dictionary configuration and auto-coding settings';

-- =====================================================================================================================
-- TABLE 4: study_form_data_reviews
-- 
-- Purpose: Track SDV, medical review, and data review workflow
-- Partition: By study_id (HASH, 16 partitions)
-- 
-- Features:
...
-- =====================================================================================================================
CREATE TABLE IF NOT EXISTS study_form_data_reviews (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    study_id BIGINT NOT NULL,
    subject_id VARCHAR(50) NOT NULL COMMENT 'Subject/Patient identifier',
    visit_id BIGINT NOT NULL COMMENT 'Reference to visit instance',
    form_id BIGINT NOT NULL COMMENT 'Form being reviewed',
    field_name VARCHAR(100) COMMENT 'Specific field (NULL = entire form)',
    
    -- Review Type
    review_type ENUM('SDV', 'MEDICAL_REVIEW', 'DATA_REVIEW', 'SAFETY_REVIEW', 'CENTRAL_REVIEW') NOT NULL,
    
    -- Review Status
    review_status ENUM('PENDING', 'IN_PROGRESS', 'COMPLETED', 'QUERY_RAISED', 'RESOLVED') DEFAULT 'PENDING',
    
    -- Review Details
    reviewer_id BIGINT COMMENT 'User who performed review',
    reviewer_name VARCHAR(255),
    reviewer_role VARCHAR(50) COMMENT 'Role of reviewer (PI, CRA, Data Manager, etc.)',
    review_date TIMESTAMP COMMENT 'When review was completed',
    review_duration_minutes INT COMMENT 'Time spent on review',
    
    -- Review Outcome
    review_outcome ENUM('PASS', 'FAIL', 'QUERY', 'NOT_APPLICABLE') COMMENT 'Result of review',
    discrepancy_found BOOLEAN DEFAULT FALSE,
    discrepancy_description TEXT,
    
    -- Query Management (if query raised)
    query_id BIGINT COMMENT 'Reference to query if raised',
    query_status ENUM('OPEN', 'ANSWERED', 'CLOSED', 'CANCELLED'),
    query_priority ENUM('LOW', 'MEDIUM', 'HIGH', 'CRITICAL'),
    ...
    -- Verification
    verified_against_source BOOLEAN DEFAULT FALSE COMMENT 'For SDV: verified against source documents',
    source_document_type VARCHAR(100) COMMENT 'Type of source document',
    source_document_reference VARCHAR(255) COMMENT 'Reference to source document',
    
    -- Electronic Signature (for 21 CFR Part 11)
    electronic_signature VARCHAR(500) COMMENT 'Encrypted signature if required',
    signature_meaning VARCHAR(100) COMMENT 'Meaning of signature (REVIEWED, APPROVED, etc.)',
    ...
    INDEX idx_study_subject (study_id, subject_id),
    INDEX idx_study_visit (study_id, visit_id),
    INDEX idx_review_type (study_id, review_type, review_status),
    INDEX idx_pending_reviews (study_id, review_status, review_type),
    INDEX idx_reviewer (study_id, reviewer_id, review_date),
    INDEX idx_query_status (study_id, query_status),
...
-- PARTITION BY HASH(study_id) PARTITIONS 16
COMMENT='SDV and data review workflow tracking for regulatory compliance';

-- =====================================================================================================================
-- AUDIT TRIGGERS for study_field_metadata
-- =====================================================================================================================

-- Audit table for field metadata changes
CREATE TABLE IF NOT EXISTS study_field_metadata_audit (
    audit_id BIGINT PRIMARY KEY AUTO_INCREMENT,
    id BIGINT NOT NULL,
    study_id BIGINT NOT NULL,
    form_id BIGINT NOT NULL,
    ...
    INDEX idx_audit_field (study_id, form_id, field_name),
