package com.clinprecision.siteservice.site.projection;

import com.clinprecision.siteservice.repository.*;
...
import com.clinprecision.siteservice.site.event.SiteActivatedEvent;
import com.clinprecision.siteservice.site.event.SiteCreatedEvent;
import com.clinprecision.siteservice.site.event.SiteUpdatedEvent;
import com.clinprecision.siteservice.site.event.UserAssignedToSiteEvent;
    ...
    public void on(SiteCreatedEvent event) {
        ...
        try {
            ...
            if (event.getOrganizationId() != null) {
                ...
                try {
                    ResponseEntity<OrganizationDto> response = organizationServiceClient.getOrganizationById(event.getOrganizationId());
                    if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                        OrganizationEntity organization = organizationMapper.toEntity(response.getBody());
                        site.setOrganization(organization);
                    }
                } catch (Exception e) {
                    System.out.println("[PROJECTION] WARNING: Failed to fetch organization with ID " + event.getOrganizationId() + ": " + e.getMessage());
                    // Continue without setting organization - don't fail the entire event
                }
            }
        ...
        try {
            ...
            if (event.getOrganizationId() != null) {
                ...
                try {
                    ...
                    System.out.println("[PROJECTION] WARNING: Failed to fetch organization with ID " + event.getOrganizationId() + ": " + e.getMessage());
                    // Continue without updating organization - don't fail the entire event
                }
            }
